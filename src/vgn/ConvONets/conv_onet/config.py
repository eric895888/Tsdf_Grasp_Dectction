...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ...
    detach_tsdf = 'detach_tsdf' in cfg.keys() and cfg['detach_tsdf']  #GIGADetach()才有

    if tsdf_only:    #GIGAGeo會用這一個
        decoders = []
    else: #GIGA會用這一個 表示跟原始vgn一樣的輸出 
    ...
    if cfg['decoder_tsdf'] or tsdf_only:  #GIGAGeo會用這一個  
    ...
    elif encoder is not None:   #像是voxel_simple_local GIGA會用這一個
