...
def main(args):
    ...
    train_writer, val_writer ,test_writer= create_summary_writers(net, device, logdir)
...
def create_train_val_test_loaders(root, root_raw, batch_size, val_split, augment, kwargs):
    # load the dataset
    ...
    train_set, val_set, test_dataset = torch.utils.data.random_split(dataset, [train_size, val_size, test_size])
...
def create_summary_writers(net, device, log_dir):
    ...
    test_writer = tensorboard.SummaryWriter(test_path, flush_secs=60)
