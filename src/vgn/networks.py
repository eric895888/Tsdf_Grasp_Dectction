...
def GIGA():  #這裡程式碼的定義怪怪的,依照ConvONets的paper定義 pointnet應該為encoder 且 unet應該為decoder,但程式這邊的定義應該是把unet定義為encoder跟剩餘後半部為decoder
    config = {
        'encoder': 'voxel_simple_local',
        'encoder_kwargs': {
            'plane_type': ['xz', 'xy', 'yz'],
...
if __name__ == "__main__":
    ...
    # # 生成隨機輸入
    # random_input = torch.randn(batch_size, 1, D, H, W)
    # net=GIGA()
    # summary(net.to(device), random_input)
    from torchview import draw_graph

    model = GIGA()

    # device='meta' -> no memory is consumed for visualization
    model_graph = draw_graph(model, input_size=(batch_size, 1, D, H, W), device='meta')
